...
const foodImagesId = ref('');
const myFile = ref();

onMounted(async () => {
    if (route.query.id) {
        foodImagesId.value = route.query.id.toString();
    }
    if (foodImagesId.value) {
        try {
            store.dispatch(SET_LOADING, true);
            let { data } = (
                await http.get(
                    `/FoodImages/GetById?recordId=${foodImagesId.value}`
                )
            ).data;
            models.value = {
                foodId: data?.foodId,
    ...
    if (errors.value.length == 0) {
        ...
        uploadTask.on(
            ...
            async () => {
                ...
                try {
                    const payload = {
                        foodImageId: foodImagesId.value,
                    ...
                    const { data } = await http.put(
                        '/FoodImages/updateRecord',
                        JSON.stringify(payload)
                    );
                    if (data.success) {
                        notify('Edit success!', TypeToast.success);
                        router.push({ path: `/admin/foodImages` });
                    }
                    store.dispatch(SET_LOADING, false);
                } catch (error) {
                    console.log(error);
